...
def _expand_line(doc_root, line):
    ...
    if m:
        ...
        if inc_file.endswith('.shtml'):
            ...
            with open(os.path.join(doc_root, "templates", inc_file), 'r', encoding='utf-8') as fp:
                inc_content = fp.read()
            if inc_content[-1] != '\n':
...
def _expand_shtml(doc_root, fs):
    ...
    '''
    with open(os.path.join(doc_root, "sources", fs), 'r', encoding='utf-8') as fp:
        lines = fp.readlines()
...
def generate(doc_root, rel_path, force_override):
    ...
    '''
    for f in os.listdir(os.path.join(doc_root, "sources", rel_path)):
        if os.path.isdir(os.path.join(doc_root, "sources", rel_path, f)):
